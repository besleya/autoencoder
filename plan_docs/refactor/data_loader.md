...
3. Call `batch->gather()`, then — only if step 1 marked this as the chunk's last full slice — call `start_next_chunk_decode_async()`, then call `batch->send_to_device()`. No callback is needed: `to_device()` is fire-and-forget (`cudaMemcpyAsync`, never blocks on the GPU — confirmed by inspecting the implementation), so sandwiching the next-chunk kickoff between the two plain calls carries no timing risk. This is simpler than the callback-based design considered earlier and was chosen for that reason. See "Ordering up the next chunk" below.
...
- `gather()` (see [batch.md](batch.md)) is the only step that reads from the `Chunk` (it copies `row_idx`/`values` for this batch's columns into the slot's own pinned buffers). Once `gather()` returns, that `Batch` will never touch `chunk_` again — `send_to_device()` only reads from the pinned buffers `gather()` just wrote, and it never blocks on the GPU (`cudaMemcpyAsync` only).
- So "the last Batch from the chunk no longer needs it" == "the batch that claimed the chunk's final full slice has finished its `gather()` call" — which is *before* that batch's H2D copy or GPU event have even started, let alone before it's consumed by the trainer.
- Because `send_to_device()` is async and non-blocking, `DataLoader::fill()` can call `gather()`, then `start_next_chunk_decode_async()` (only for the last-slice batch), then `send_to_device()`, as three plain sequential calls — no callback plumbing through `Batch` is needed. (An earlier draft of this plan added an `after_gather` callback parameter to `Batch::prepare()` for this; that was dropped once inspecting `to_device()` confirmed it's fire-and-forget, since the callback version was strictly more complex for no benefit.)
- `start_next_chunk_decode_async()` submits the per-file decode tasks to `ring_->decode_pool()` and stores the resulting `std::shared_future<Chunk>` in `next_chunk_future_` — it does **not** call `.get()`, so the calling `fill_pool_` worker proceeds straight on to `send_to_device()` for the current (last-of-chunk) batch without waiting. That batch becomes ready on the normal schedule; decoding of the next chunk proceeds in the background on `decode_pool_`.
